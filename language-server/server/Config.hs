...
data Config = Config {
  ...
  -- Cryptol state (e.g., loading modules). We do this instead of taking
  -- the `stateRef` so that the server can still read the state while
  -- we are working on computing the new Cryptol state.
}
