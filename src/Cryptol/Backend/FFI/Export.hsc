...
data Mk =
  Mk { mkPrec :: Int, mkPP :: [Doc] -> Doc, mkVal :: [Value] -> Value }
    ...
    f : more ->
      case f of
        ...
        NeedMany mk vs ts ->
          let done = v : vs in
          case ts of
            [] -> haveValue (mkVal mk (reverse done)) more
          ...
          case opts Vector.!? n of
            ...
            Just ci ->
              case Vector.toList (conFields ci) of
                [] -> haveValue (mkV []) fs
                t : ts -> needValue t (NeedMany (Mk 10 ppV mkV) [] ts : fs) 
              where
...
needValue tval fs =
  case tval of
    ...
    TVSeq len elT ->
      case elT of
        ...
        _ | len < 1 -> haveValue (mkV []) fs
          | otherwise ->
            needValue elT (NeedMany (Mk 0 ppV mkV) [] ts : fs) 
        where
    ...
    TVTuple tys ->
      case tys of
        [] -> haveValue (mkV []) fs
        t : ts -> needValue t (NeedMany (Mk 0 ppV mkV) [] ts : fs)
        where
