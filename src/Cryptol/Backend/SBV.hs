...
fpCvtToInteger sym fun r x = do
  ...
  case rnd of
    ...
    SBV.RoundNearestTiesToAway -> pure $ svRealRoundAway y
...
fpRealRoundEven sym x
  ...
  | otherwise
  = do nm <- liftIO $ freshSInteger_ sym
       let r = svFromIntegral KReal x
       -- TODO RGS: Docs
...
-- TODO RGS: Note that KindCast uses floor under the hood
...
-- TODO RGS: Docs
svRealRoundAway :: SBV.SVal -> SInteger SBV
svRealRoundAway x =
  let isNeg = svLessThan x (svReal 0) in
  let half = svReal (1 % 2) in
  svIte isNeg (svRealCeil (svMinus x half)) (svRealFloor (svPlus x half))
