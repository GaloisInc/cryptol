...
data ModuleG mname =
              Module { mName             :: !mname
                     ...
                     , mIsIfaceFunctor   :: !Bool
                       -- ^ True when this is an interface functor
                       -- (parameterized interface) rather than a
                       -- normal functor.

                     -- Functors:
                     , mParamDecls       :: ParamDecls
                       -- ^ Input parameters: type/value parameters and
                       -- constraints from interface imports.  These are
                       -- resolved when the functor is instantiated.
                       -- Use by normal and interface functors.
                     ...
                     , mOutputParamDecls :: ParamDecls
                       -- ^ Output parameters: declared by interfaces
                       -- (both ordinary and functor interfaces).
                       -- These specify what an interface provides.
                     ...
                     , mFunctors         :: Map Name (ModuleG Name)
                       ...
                       -- modules in the elements of the map.
                       -- This includes both regular functors and interface
                       -- functors; use 'mIsIfaceFunctor' on each entry to
                       -- distinguish.
...
data Submodule = Submodule
  ...
  , smInScope :: NamingEnv
  , smVirtual :: Bool
    -- ^ Generated by the system (e.g., to expose functor parameters).
  } deriving (Show, Generic, NFData)

mParamTypes :: ModuleG mname -> Map Name ModTParam
mParamTypes = pdTypes . mParamDecls

mParamFuns :: ModuleG mname -> Map Name ModVParam
mParamFuns = pdFuns . mParamDecls

mParamConstraints :: ModuleG mname -> [Located Prop]
mParamConstraints = pdConstraints . mParamDecls
...
instance PP n => PP (WithNames (ModuleG n)) where
  ppPrec _ (WithNames Module { .. } nm) =
    withPPCfg $ \cfg ->
      let
        mps = map mtpParam (Map.elems (pdTypes mParamDecls) ++
                                       Map.elems (pdTypes mOutputParamDecls))
    ...
    vcat [
      ...
      catMaybes
           -- XXX: Print exports?
           [ vcat' [ "-- Input parameters:" $$ indent 2 (pp mParamDecls)
                   | not (null (pdTypes mParamDecls))
                     || not (null (pdFuns mParamDecls)) ]
           , vcat' [ "-- Output parameters:" $$ indent 2 (pp mOutputParamDecls)
                   | not (null (pdTypes mOutputParamDecls))
                     || not (null (pdFuns mOutputParamDecls)) ]
           , Just ""
