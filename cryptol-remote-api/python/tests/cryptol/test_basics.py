...
class BasicServerTests(unittest.TestCase):
    ...
    def test_check_timeout(self):
        ...
        with self.assertRaises(ArgoException):
            c.check("\\(bv : [256]) -> ~ (~ (~ (~bv))) == bv", num_tests="all", timeout=1.0)
        t2 = time.time()
        self.assertLess(t2 - t1, 2.0)

        t1 = time.time()
        with self.assertRaises(ArgoException):
            c.check("\\(bv : [256]) -> ~ (~ (~ (~bv))) == bv", num_tests="all", timeout=5.0)
        t2 = time.time()
        self.assertLess(t2 - t1, 7)

        t1 = time.time()
        c.check("\\(bv : [256]) -> ~ (~ (~ (~bv))) == bv", num_tests=10, timeout=5.0)
        t2 = time.time()
    ...
    def test_interrupt(self):
        ...
        # ^  intentionally omitted so we don't wait on it's result and we can interrupt
        # it on the next line.
        time.sleep(.5)
        c.interrupt()
    ...
    def test_prove_timeout(self):
        ...
        self.assertTrue(c.check("AESCorrect").success)
        t1 = time.time()
        with self.assertRaises(ArgoException):
            c.prove("AESCorrect", timeout=1.0)
        t2 = time.time()
        ...
        with self.assertRaises(ArgoException):
            ...
            c.prove("AESCorrect", timeout=5.0)
        t2 = time.time()
