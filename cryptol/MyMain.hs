...
main :: IO ()
main  = do
  REPL.runREPL True {- is batch -}
          False {- call stacks -}
          stdoutLogger
          action
 where
  action =
    do REPL.setSearchPath ["."]
       code <- crySession (Batch "sort.icry") True
       return ()

       -- REPL.moduleCmd "collatz"
       -- menv <- REPL.getModuleEnv
       -- REPL.io (print (meSearchPath menv))
